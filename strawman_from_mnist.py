...
def main(_):
  # Import data
  kaggle_planet = read_data_sets()
  print("Imported data")

  # Create the model
  x = tf.placeholder(tf.float32, [None, Image.SIZE])
  ...
  # the sigmoid cross entropy function is appropriate for multilabel problems
  # (unlike softmax where labels are mutually exclusive)
  cross_entropy = tf.reduce_mean(
      tf.nn.sigmoid_cross_entropy_with_logits(targets=y_, logits=y_conv))
  train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
  # The model is evaluated based on the F2 score, which would require setting a cutoff for each
  # label etc. For now we skip this.
  # correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
  # accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))

  with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    for i in range(500):  # No need for too many steps with this simple model...
      batch = kaggle_planet.train.next_batch(50)
      if i % 100 == 0:
        print('step {}, cross entropy: {}'.format(i, 
          cross_entropy.eval(feed_dict={x: batch[0], y_: batch[1]})))
